...
def getFoodHeuristic(gameState):
  ...
  """
  Constructs a graph where each position is connected to positions adjacent to it that are not walls
  """
  global walls
...
def foodHeuristic(state):
  ...
  """
  Heuristic computes the cost of an MST going through all of the food as well as pacman's current position
  Distances between nodes are stored in dist dictionary using Floyd Warshall algorithm, to go around walls
  instead of the more optimistic Manhattan distance
  Unfortunately, for this specific case, this better heuristic of using BFS distances instead of Manhattan
  distance doesn't end up expanding fewer nodes (although it could in many cases) (Extra credit would be appreciated!:P)
  """

  currentPosition = state[0]
  foodPositions = state[1].asList()
  vertices = {}
  vertices[currentPosition] = True
  distance = {}
  visited = {}
  Q = util.PriorityQueue()
  Q.push(currentPosition, 0)
  distance[currentPosition] = 0
  for position in foodPositions:
    distance[position] = float("inf")
    vertices[position] = True
  while not Q.isEmpty():
    v = Q.pop()
    visited[v] = True
    for neighbor in vertices:
      if neighbor != v and neighbor not in visited:
        if distance[neighbor] > dist[(v, neighbor)]:
          distance[neighbor] = dist[(v, neighbor)]
...
class AStarFoodSearchAgent(SearchAgent):
  ...
  def __init__(self, searchFunction=None, searchType=FoodSearchProblem):
    ...
    SearchAgent.__init__(self, self.searchFunction, searchType)


  # Overriding this function to be able to pass in the gameState to getFoodHeuristic
  def registerInitialState(self, state):
    """
    This is the first time that the agent sees the layout of the game board. Here, we
    choose a path to the goal.  In this phase, the agent should compute the path to the
